...
export default function AddProductScreen() {
    ...
    const [returnPolicy, setReturnPolicy] = useState('return');
    const [returnWindow, setReturnWindow] = useState('7');

    // Custom Attributes (Brand, Model, etc.)
    ...
    const handleSubmit = async () => {
        ...
        // Base price only required for simple products, not variants
        ...
        if (hasVariants) {
            const invalidVariants = generatedVariants.filter(v => !v.stock || Number(v.stock) <= 0);
            if (invalidVariants.length > 0) {
                Alert.alert('Invalid Stock', 'Please set stock quantity for all variants');
                return;
            }
        }
        ...
        try {
            ...
            const uploadedPaths = uploadRes.data;

            // 2. Prepare custom attributes
            const attrs: any = {};
            ...
            await api.post('/products', {
                ...
                returnPolicy,
                returnWindow: returnPolicy === 'no_return' ? 0 : Number(returnWindow)
            });
            ...
            <ScrollView contentContainerStyle={styles.content}>
                ...
                <View style={styles.pickerBox}>
                    <Picker
                        ...
                        style={{ height: 50, color: categoryId ? '#333' : '#999' }}
                    >
                    ...
                    <>
                        ...
                        <View style={styles.pickerBox}>
                            <Picker
                                ...
                                style={{ height: 50, color: subcategory ? '#333' : '#999' }}
                            >
                ...
                <View style={styles.toggleRow}>
                    <TouchableOpacity
                        ...
                        onPress={() => { setHasVariants(false); setGeneratedVariants([]); }}
                    >
                    ...
                    <View style={styles.variantsSection}>
                            ...
                            <View key={vtIdx} style={styles.variantTypeBox}>
                                <View style={styles.variantTypeHeader}>
                                    <TextInput
                                        ...
                                        placeholderTextColor="#999"
                                        value={vt.name}
                                        onChangeText={(text) => updateVariantTypeName(vtIdx, text)}
                                    />
                                ...
                                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                                    <TextInput
                                        ...
                                        placeholder={`Add option (e.g., ${vt.name || 'value'})`}
                                        placeholderTextColor="#999"
                                        value={tempOptionInputs[vtIdx] || ''}
                                        onChangeText={(text) => {
                                            setTempOptionInputs({ ...tempOptionInputs, [vtIdx]: text });
                                        }}
                                        onSubmitEditing={() => {
                        ...
                        {variantTypes.length > 0 && (
                            <TouchableOpacity style={styles.generateBtn} onPress={generateVariants}>
                                <Text style={styles.generateBtnText}>GENERATE VARIANTS</Text>
                            </TouchableOpacity>
                        )}

                        {/* Quick Price Setup - NEW FEATURE */}
                        {generatedVariants.length > 0 && (
                            <View style={styles.quickPriceBox}>
                                <Text style={styles.subTitle}>⚡ Quick Price Setup</Text>
                                <Text style={styles.hint}>Set all prices at once with auto-increment</Text>
                                <View style={{ flexDirection: 'row', gap: 8, marginTop: 10 }}>
                                    <TextInput
                                        style={[styles.input, { flex: 1 }]}
                                        placeholder="Start Price (e.g., 500)"
                                        placeholderTextColor="#999"
                                ...
                                >
                                    <Text style={styles.applyPriceBtnText}>Apply to All Variants</Text>
                                </TouchableOpacity>
                            </View>
                        )}
                        ...
                        {generatedVariants.length > 0 && (
                            <View style={styles.generatedSection}>
                                <Text style={styles.subTitle}>Generated Variants ({generatedVariants.length})</Text>
                                    ...
                                    <View key={idx} style={styles.variantCard}>
                                        ...
                                        <View style={styles.variantInputs}>
                                            <TextInput
                                                ...
                                                keyboardType="numeric"
                                                value={v.price}
                                                onChangeText={(text) => updateVariantField(idx, 'price', text)}
                                            />
                                            <TextInput
                                                style={[styles.input, { flex: 1 }]}
                                                placeholder="Stock"
                                                ...
                                                keyboardType="numeric"
                                                value={v.stock}
                                                onChangeText={(text) => updateVariantField(idx, 'stock', text)}
                                            />
                                        </View>
                                    </View>
                                ))}
                ...
                <Text style={styles.label}>
                    {hasVariants ? 'Base Price (₹) - Used as default for variants' : 'Price (₹) *'}
                </Text>
                ...
                <TextInput
                    style={styles.input}
                    placeholder="Shipping Cost (Default 40)"
                    placeholderTextColor="#999"
                    keyboardType="numeric"
                    value={shippingCost}
                ...
                <View style={styles.pickerBox}>
                    <Picker
                        selectedValue={returnPolicy}
                        onValueChange={setReturnPolicy}
                        style={{ height: 50, color: '#333' }}
                    >
                        <Picker.Item label="Return & Replace" value="return" />
                        <Picker.Item label="Replacement Only" value="replacement" />
                        <Picker.Item label="No Return" value="no_return" />
                    </Picker>
                </View>
                ...
                {/* Custom Attributes */}
                <Text style={styles.sectionTitle}>Custom Attributes (Optional)</Text>
                <Text style={styles.hint}>Add Brand, Model, Fabric, etc.</Text>
                ...
                {customAttributes.map((attr, idx) => (
                    <View key={idx} style={styles.attrRow}>
                        <TextInput
                            ...
                            placeholderTextColor="#999"
                            value={attr.name}
                            onChangeText={(text) => updateCustomAttribute(idx, 'name', text)}
                        />
                        <TextInput
                            style={[styles.input, { flex: 1, marginRight: 8 }]}
                            placeholder="Value"
                            placeholderTextColor="#999"
                            value={attr.value}
                            onChangeText={(text) => updateCustomAttribute(idx, 'value', text)}
                        />
...
const styles = StyleSheet.create({
    ...
    postBtn: { color: '#FF6600', fontSize: 16, fontWeight: 'bold' },
    content: { padding: 16 },
